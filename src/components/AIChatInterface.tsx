...
import { fixedAIService } from '../services/fixedAIService';
...
export const AIChatInterface: React.FC<AIChatInterfaceProps> = ({
  ...
  // Initialize messages with welcome message - FIXED: Only initialize once
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome-' + Date.now(),
      content: "Hi! I'm your AI fitness coach with expertise in strength training, nutrition, and motivation. I can also calculate your BMI and provide personalized advice. What would you like to know?",
      isUser: false,
      timestamp: new Date()
    }
  ]);
  ...
  // FIXED: Prevent duplicate message additions with ref tracking
  ...
  // FIXED: Simplified message addition without duplication
  const addMessage = useCallback((content: string, isUser: boolean, metadata?: any) => {
    ...
    setMessages(prevMessages => {
      ...
      if (exists) {
        console.log('🚫 Duplicate message prevented:', content.substring(0, 30) + '...');
        return prevMessages;
      }
  ...
  const handleSendMessage = async (text: string) => {
    if (!text.trim() || messageProcessingRef.current) return;

    console.log('🚀 Sending message to AI:', text);
    
    // FIXED: Prevent concurrent message processing
    ...
    try {
      ...
      } else if (lowerText.includes('workout') || lowerText.includes('routine') || lowerText.includes('exercise')) {
        requestType = 'workout-planning';
      }

      console.log('🎯 Request type:', requestType);

      // Call the fixed AI service
      const aiResponse = await fixedAIService.getCoachingResponse(
        text, 
        workoutContext || {}, 
        requestType as any
      );
      ...
      // FIXED: Only add AI response once
      ...
      const fallbackResponse = "I'm having some technical difficulties, but I'm still here to help! Try asking me about workouts, nutrition, motivation, or BMI calculation.";
      addMessage(fallbackResponse, false, { provider: 'error_fallback' });
    } finally {
      setIsLoading(false);
      ...
      <div className="p-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm opacity-90">Expert guidance + BMI calculator</p>
          </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          <input
            ...
            placeholder={isVoiceMode ? "Listening..." : "Ask about fitness, nutrition, motivation, or BMI..."}
          ...
          <button
            type="submit"
            disabled={!inputText.trim() || isLoading || isVoiceMode || messageProcessingRef.current}
