...
const ClassEnrollment: React.FC<ClassEnrollmentProps> = ({ classes }) => {
  ...
  const ActionButton: React.FC<{c: Class}> = ({ c }) => {
    ...
    return <button className="w-full sm:w-auto bg-[var(--color-accent)] text-black font-bold py-2 px-4 rounded-lg text-sm transition-transform hover:scale-105">Join</button>
  }
