...
export const useStreamingAI = (options: UseStreamingAIOptions = {}) => {
  ...
  const streamResponse = useCallback(async (message: string) => {
    ...
    try {
      ...
      // Equipment-only intent (e.g., "Gym Membership", "At Home", "Outdoors")
      const mentionsEquipmentOnly = (
        /(gym|membership|at home|home|outdoor|outdoors|outside|not sure|unsure)/.test(lowerMessage) &&
        !(
          ...
          /\b\d+\s*-?\s*week/.test(lowerMessage)
        ) &&
        !(
      ...
      if (mentionsEquipmentOnly) {
        const equipment = lowerMessage.includes('gym') || lowerMessage.includes('membership')
          ? 'gym'
          : lowerMessage.includes('home')
            ? 'home'
            : (lowerMessage.includes('outdoor') || lowerMessage.includes('outside'))
              ? 'outdoors'
              : 'unsure';
        responses = [
          `Great — equipment preference noted: ${equipment === 'gym' ? 'Gym Membership' : equipment === 'home' ? 'At Home' : equipment === 'outdoors' ? 'Outdoors' : 'Not Sure Yet'}. `,
          `To tailor your plan, what's your experience level, primary goal, and available time per session (minutes)? `,
          `You can choose an option or type your answer — experience, goal, and time.`
        ];
        const fullImmediate = responses.join('');
        if (onChunk) onChunk(fullImmediate);
        if (onComplete) onComplete(fullImmediate);
