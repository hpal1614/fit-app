...
import type { WorkoutDay } from '../../types/finalUI';
import { WorkoutStatus } from '../../types/finalUI';
...
const DateSelector: React.FC<DateSelectorProps> = ({ week, selectedDayIndex, onDaySelect }) => {
  ...
  const isOnToday = selectedDayIndex === todayIndex;
  ...
  const getStatusClasses = (status: WorkoutStatus, isSelected: boolean, isToday: boolean) => {
    ...
    if (isToday && status === WorkoutStatus.Upcoming && !isSelected) {
        classes += ' ring-2 ring-offset-2 ring-offset-gray-900 ring-lime-400';
    }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-between items-center px-2">
          <h3 className="text-xl font-semibold text-gray-100">Your Week</h3>
                     <button onClick={handleGoToToday} className={`flex items-center space-x-2 text-sm font-semibold text-gray-900 bg-gray-200 px-3 py-1.5 rounded-lg transition-all duration-200 hover:bg-[#334a18] hover:text-[#bdf164] hover:font-bold active:bg-[#a4e635] active:text-black active:font-bold ${isOnToday ? 'bg-[#a4e635] text-black font-bold' : ''}`}>
