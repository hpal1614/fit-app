...
import { 
    Send, X, Bot, Sparkles, Brain, MessageCircle,
  Zap, Plus, Trash2
} from 'lucide-react';
...
export const IntegratedAICoach: React.FC<IntegratedAICoachProps> = ({
  ...
  const [, setCurrentStreamId] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'research' && ragInitialized) {
        ...
        fullResponse = "RAG system temporarily disabled";
        sources = [];
        confidence = 0.5;
      } else {
        ...
        // Just wait for the stream to complete - tokens are handled in onToken
        for await (const _ of stream) {
          // Tokens are already processed in onToken callback
        }
      }
