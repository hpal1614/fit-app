...
interface WorkoutPlannerProps {
  ...
  onAddToHome: (template: WorkoutTemplate) => void;
  onBack: () => void;
}
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            onClick={async () => {
              if (selectedTemplate) {
                await workoutStorageService.saveWorkoutTemplate(selectedTemplate as any);
                await workoutStorageService.activateWorkoutTemplate(selectedTemplate.id);
                await workoutStorageService.generateWeeklySchedule(selectedTemplate as any);
                onAddToHome(selectedTemplate);
              }
            }}
            className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 px-6 rounded-lg font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 flex items-center justify-center space-x-2"
          >
