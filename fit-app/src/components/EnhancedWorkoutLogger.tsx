...
Coach: "Great! I've updated it to ${context.lastSetWeight + 5} lbs. You've got this! Focus on form and push through.`
  ...
  // Load workout template data when workout prop changes
  useEffect(() => {
    if (workout && workout.exercises && workout.exercises.length > 0) {
      console.log('Loading workout template:', workout);
      
      // Convert workout template exercises to the format expected by the logger
      const templateExercises = workout.exercises.map((exercise: any, index: number) => ({
        id: index,
        name: exercise.name,
        sets: exercise.sets,
        reps: exercise.reps,
      ...
      // Reset exercise states for new workout
      const newExerciseStates: any = {};
      templateExercises.forEach((exercise: any) => {
        newExerciseStates[exercise.id] = {
          weight: 135, // Default weight
          reps: 8,
          rpe: 7,
          completedSets: 0,
          history: []
      ...
      console.log('Workout template loaded:', templateExercises);
    }
  }, [workout]);
      ...
      <div className="card card-elevated">
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="text-xs text-gray-400 font-medium tracking-wider uppercase">
                {workout?.name || 'Workout'} • Exercise {currentExerciseIndex + 1}/{workoutExercises.length}
              </div>
